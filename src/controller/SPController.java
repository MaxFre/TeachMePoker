...
//	private String winner = "";
	...
	/**
	 * Method which receives and sets a number of starting variables and for the
	 * game to be set up.
	 * 
	 * @param noOfAi
	 *            Number of AI-players
	 * @param potSize
	 *            The potsize for the table(game).
	 * @param playerName
	 *            The players' name.
	 */
	...
	/**
	 * Method which sets a GameController, the controller that controls the GUI
				...
				if (currentPlayer == noOfPlayers - 1) {
					...
					if (!aiPlayers.get(currentPlayer).getDecision().contains("lost")) {
						if (!aiPlayers.get(currentPlayer).getDecision().contains("fold")) {
							...
							if (!(checkLivePlayers() > 1)) {
								System.out.println(aiPlayers.get(currentPlayer).getName() + " Wins " + currentPotSize);
	...
	/**
	 * Method which checks the amount of "living" players. The amount of players
	 * whose decision is not fold.
	 * 
	...
	/**
	 * Method which asks the GUi to give the player a choice and calls an action
	 * when a decision has been made.
	 * 
	 * @param currentMaxBet2
	 *            the currentmaxbet.
	 */
	...
	/**
	 * Method which asks the current AIplayer to make a decision based on the
	 * current max bet.
	 */
	...
	/**
	 * Method which sets who the small and big blind players are. Depending on
	 * who the dealer is.
	 * 
	 * @param noOfPlayers
	...
	private void setBlinds(int noOfPlayers) {
		...
		while (bigBlindPlayer != (noOfPlayers - 1) && aiPlayers.get(bigBlindPlayer).getDecision().contains("lost")) {
			bigBlindPlayer = (bigBlindPlayer + 1) % noOfPlayers;
		}
	...
	public void allCallorFold() {
		...
		for (Ai ai : aiPlayers) {
			...
			if (ai.getDecision().contains("fold")) {
				...
				noOfAIFoldedorCalled++;
				// if neither checked, called or folded, at least one AI is
				// live.
			} else {
				allCalledorFolded = false;
			}
		}
		// If all AI have folded or called, check if player has folded or
		// called.
