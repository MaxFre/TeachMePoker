...
public class NewGameMenuGUI extends Application {
  ...
  public void start(Stage primaryStage) throws Exception {
    ...
    sliderPot.setOnMouseDragged(
        e -> lblPotinfo.textProperty().setValue(String.valueOf((Double) sliderPot.getValue())));
    ...
    sliderAI.setOnMouseDragged(
        e -> lblAIinfo.textProperty().setValue(String.valueOf((Double) sliderAI.getValue())));



    grid.setConstraints(lblAIinfo, 1, 2);

    grid.getChildren().addAll(lblName, tfNameInput, lblAI, sliderAI, lblPot, sliderPot, lblPotValue,
        lblPotinfo, lblAIValue, lblAIinfo);
