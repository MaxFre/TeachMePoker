...
public class GameController {
	 ...
	 * @param state The state can either be inactive (folded/lost), idle (waiting for it's turn), active (currently it's turn).
	 */
	...
	/**
	 * Disables all buttons and shows player-frame's action as call, and the called amount. Calculates and withdraws amount from player-pot.
	 */
	public void playerCall() {

		disableButtons();
		System.out.println("Player call");
		this.playerPot -= (spController.getCurrentMaxBet() - alreadyPaid); // Player's pot - (Current maxbet - already paid (prev rounds)) = THE PLAYER'S POT 
	...
	/**
	 * Disables all buttons and shows player-frame's action as raise, and the raised amount. Calculates and withdraws amount from player-pot and adjusts already paid.
	 */
	...
	/**
	 * Sets the slider's min and max values based on the player-pot. Sets minimum sliderValue based on BigBlind.
	 */
...
//		sound.stopSound();
	...
	/**
	 * Checks the player's hand and gives tips and highlights cards based on the method getHighlightedCards (important during pre-flop situation).
	 */
	...
	public void setFlopTurnRiver(Card[] setOfCards) {
		...
		Platform.runLater(() -> {
			...
			int xCord = 0;
			for (int i = 0; i < setOfCards.length; i++) { // Loops through all cards and highlights the correct ones and places them on the table (UI)
				String baseCard = "";
				if (hand.getHighlightedCards().contains(Integer.toString(setOfCards[i].getCardValue()) + "," + setOfCards[i].getCardSuit().charAt(0) )) {
	...
	public void aiAction(int currentAI, String decision) {
		...
		//System.out.println("Current position: " + currentAIPosition + "  SET: " + setOfPlayers + "  currentAI: " + currentAI + "   prevActivePlayer: " + prevPlayerActive + "   currentAIPosition: " + currentAIPosition);

		if(prevPlayerActive != -1){ // If there does exists a previous active AI-player
			setUIAiStatus(prevPlayerActive, "idle"); //Resets the previous player's image from glowing(active) to non-glowning(idle) 
		}
