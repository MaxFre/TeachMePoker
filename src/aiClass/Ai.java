...
/**
 * Makes the decision for the last two turns of the game.
 * If there are less than 7 cards its the second last turn and it calls for a calculation on that turn.
 * And if its 7 cards its the last turn and it does a calculation for that turn instead.
 * So never two calculations och one call.
 * @param currentBet How much the Ai-player as to bet to be able to play this turn.
 ...
 */
  ...
  /**
   * Makes sure that AI-players decision from last this
   *  isnt making a problem for current turns decision.
  ...
  /**
   * If ai-player wins the round this gets updated with 
   * the winning amount added to its current potsize.
...
/**
 * Checks if ai-player is bigblind or not and if it is. 
 * It takes a amount from the ai's pot
 ...
 */
  ...
  /**
   * Checks if ai-player is smallBlind or not and if it is. 
   * it takes a amount from the ai's pot
